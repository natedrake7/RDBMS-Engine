...
public:
    ...
    static void SetBit(UnsignedTinyInt& data, const uint8_t& bitPosition, const bool& value) {
        if (value)
            data |= (1 << bitPosition);
        else
            data &= ~(1 << bitPosition);
    ...
    static bool GetBit(const UnsignedTinyInt& data, const uint8_t& bitPosition) {

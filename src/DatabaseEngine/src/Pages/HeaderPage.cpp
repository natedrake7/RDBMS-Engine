...
//            }
    ...
    void HeaderPage::ReadFromDisk(const std::vector<char> &data, const DatabaseEngine::StorageTypes::Table *table, page_offset_t &offSet, fstream *filePtr){
        std::memcpy(&this->databaseHeader->numberOfTables, data.data() + offSet, sizeof(table_number_t));
        offSet += sizeof(table_number_t);

        std::memcpy(&this->databaseHeader->lastPageFreeSpacePageId, data.data() + offSet, sizeof(page_id_t));
        offSet += sizeof(page_id_t);

        std::memcpy(&this->databaseHeader->lastGamPageId, data.data() + offSet, sizeof(page_id_t));
        offSet += sizeof(page_id_t);
        ...
        for (int i = 0; i < this->databaseHeader->numberOfTables; i++)
        {
            ...
            std::memcpy(&tableHeader.tableId, data.data() + offSet, sizeof(table_id_t));
            offSet += sizeof(table_id_t);

            std::memcpy(&tableHeader.indexAllocationMapPageId, data.data() + offSet, sizeof(page_id_t));
            offSet += sizeof(page_id_t);

            std::memcpy(&tableHeader.numberOfColumns, data.data() + offSet, sizeof(column_number_t));
            offSet += sizeof(column_number_t);

            std::memcpy(&tableHeader.clusteredIndexPageId, data.data() + offSet, sizeof(page_id_t));
            offSet += sizeof(page_id_t);

            uint8_t numberOfNonClusteredIndexes;
            std::memcpy(&numberOfNonClusteredIndexes, data.data() + offSet, sizeof(uint8_t));
            offSet += sizeof(uint8_t);

            for (int j = 0; j < numberOfNonClusteredIndexes; j++)
            {
                page_id_t pageId = 0;
                std::memcpy(&pageId, data.data() + offSet, sizeof(page_id_t));
